...
_MONITOR_CONF    = '/etc/asterisk/extensions_asl3ez_monitor.conf'
_MONITOR_CONTEXT = 'asl3ez-monitor'
_audio_lock      = threading.Lock()
_audio_active    = {}   # node -> _AudioBroadcast

_MONITOR_DIALPLAN = (
    '; Auto-generated by ASL3-EZ — do not edit\n'
    '[asl3ez-monitor]\n'
    'exten => _X.,1,Answer()\n'
    'exten => _X.,n,MixMonitor(/tmp/asl3ez_audio_${EXTEN}.sln,b)\n'
    'exten => _X.,n,Rpt(${EXTEN})\n'
    'exten => _X.,n,StopMixMonitor()\n'
    'exten => _X.,n,Hangup()\n'
)


def _ensure_monitor_dialplan():
    changed = False
    try:
        existing = open(_MONITOR_CONF).read() if os.path.exists(_MONITOR_CONF) else ''
        if existing != _MONITOR_DIALPLAN:
            with open(_MONITOR_CONF, 'w') as f:
                f.write(_MONITOR_DIALPLAN)
            changed = True
            log('INFO', '[AUDIO] Wrote monitor dialplan')
    except Exception as e:
        log('WARN', f'[AUDIO] Could not write monitor conf: {e}')

    include_line = f'#include "{_MONITOR_CONF}"'
    try:
        with open('/etc/asterisk/extensions.conf') as f:
            ext = f.read()
        if include_line not in ext:
            with open('/etc/asterisk/extensions.conf', 'a') as f:
                f.write(f'\n{include_line}\n')
            changed = True
            log('INFO', '[AUDIO] Added monitor include to extensions.conf')
    except Exception as e:
        log('WARN', f'[AUDIO] Could not update extensions.conf: {e}')
    ...
    if changed:
        def _reload(ami):
            ...
            return {'ok': True}
        try:
            ami_send_command(_reload)
            log('INFO', '[AUDIO] Dialplan reloaded')
        except Exception as e:
            log('WARN', f'[AUDIO] dialplan reload failed: {e}')
...
class _AudioBroadcast:
    ...
    def __init__(self, node, ffmpeg_proc, fifo_path, placeholder_fd):
        self.node           = node
    ...
    def shutdown(self):
        ...
        def _hup(ami):
            ami.command(
                f'channel request hangup Local/{self.node}@{_MONITOR_CONTEXT};1'
            )
            return {'ok': True}
        try:
            ami_send_command(_hup)
        except Exception:
            pass
...
def _start_broadcast(node):
    """Start ffmpeg + AMI channel; return an _AudioBroadcast. Raises on error."""
    ...
    # Open O_RDWR (non-blocking) to hold the write-end so ffmpeg's O_RDONLY
    # open doesn't block and won't see EOF before MixMonitor connects.
    ...
    broadcast = _AudioBroadcast(node, ffmpeg_proc, fifo_path, pfd)

    def _originate(ami):
        ami._send_action({
            'Action':      'Originate',
            'Channel':     f'Local/{node}@{_MONITOR_CONTEXT}',
            'Application': 'Wait',
            'Data':        '3600',
            'Async':       'yes',
            'Callerid':    'ASL3-EZ Monitor <0>',
        })
        raw = ami._recv_until('\r\n\r\n', timeout=ami.timeout)
        return ami._parse_packet(raw)

    try:
        ami_send_command(_originate)
    except Exception as e:
        broadcast.shutdown()
        raise RuntimeError(f'AMI originate failed: {e}')

    log('INFO', f'[AUDIO] broadcast started for node {node}')
    return broadcast
