...
- **Added: Status Board (`/status`).** A clean full-screen display designed for TV or kiosk use. Shows a live clock, node keyed-status indicator (large pulsing green dot when keyed, idle when not), connected nodes table with callsign and location, and system vitals (uptime, CPU temp, disk, AMI connection). Auto-refreshes every 5 seconds from a single aggregated API endpoint. Includes a Fullscreen button and a back link to the main dashboard. Accessible via "Status Board ↗" in the sidebar (opens in a new tab) or directly at `http://YOUR_NODE_IP:5000/status`.
