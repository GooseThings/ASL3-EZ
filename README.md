...
- **Added: Status Board (`/status`).** A clean full-screen display designed for TV or kiosk use. No authentication required — safe for public screens. Features:
  ...
  - **Connected nodes**: scrollable table with node number, callsign, location, and per-link keyed indicator (updated every 5 seconds via AMI).
  - **Global Activity feed**: top 10 most recently active AllStarLink nodes worldwide (polled from `stats.allstarlink.org` every 5 minutes with exponential backoff on rate-limit errors). Green dot = keyed via local AMI; blue dot = recently online globally.
  - **Network Map**: Leaflet/OpenStreetMap map with color-coded pins (green = locally keyed, blue = global). Server lat/lon comes from AllStarLink's API — no geocoding needed for global nodes.
