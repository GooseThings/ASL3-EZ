...
- **Added: Status Board (`/status`).** A clean full-screen display designed for TV or kiosk use. No authentication required — safe for public screens. Features:
  ...
  - **Global Activity feed**: top 10 most recently active AllStarLink nodes worldwide. Polls a curated list of major public hub nodes (e.g. W2ECR East Coast Reflector, M0HOY UK) every 5 minutes — each hub request is ~3 KB so this never triggers rate limits. Green dot = keyed via local AMI; blue dot = recently active globally.
  - **Network Map**: Leaflet/OpenStreetMap map with color-coded pins (green = locally keyed, blue = global nodes, gold ⭐ = this server's hosted node). Pins scale with zoom level. Server lat/lon comes directly from AllStarLink's API — no geocoding needed for global nodes. Map auto-fits on first load; user zoom/pan is preserved across all subsequent refreshes.
